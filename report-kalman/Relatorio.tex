...
\section{Introdução}
Este projeto se propõe a realizar a localização de um robô diferencial utilizando uma base, odometria e o filtro de Kalman. 
...
Por definição, o KF trabalha com probabilidades lineares. Sua versão extendida (EFK) trabalha com a hipótese de as funções que modelam as probabilidades não são lineares. Como um robô tipicamente pode realizar uma trajetória circular, o modelo mais indicado é o EKF. \ref{alg:ekf}.
...
\caption{Extended Kalman filter \(\bar{\mu_t}, \mu_{t-1}, \Sigma_{t-1}, \Sigma_{\Delta t}, z_t\)}
 \label{alg:ekf}
\begin{algorithmic} 
\STATE \(\bar{\Sigma}_{t} = G_t\Sigma_{t-1}G_t^T + V_t \Sigma_{\Delta t} V_t^T + R_t \)
...
O algoritmo do EKF está descrito pela tabela \ref{alg:ekf}, onde \(\bar{\mu_t}\) pode ser entendido como a estimativa do estado no tempo \(t\), \(\mu_{t-1}\) o cálculo do estado no tempo \(t-1\), \(\Sigma_{t-1}\) a covariância calculada em \(t-1\) e \(z_t\) são as observações no tempo t. 

\subsection{Localização com EKF}

Esta sessão é dedica a explicar como o filtro de Kalman Extendido pode ser utilizado para realizar a localização do robô com o auxílio da detecção de \textit{landmarks}. Os detalhes das fórmulas utilizadas não serão abordados, mas podem ser vistos na tabela 7.2 do livro \textit{Probabilistic Robotics}\cite{thrun2005probabilistic}.

Tratando-se de localização o estado que pretende-se prever é a pose do robô, representado, então, por \(\mu\) no algoritmo \ref{alg:ekf}. A estimativa de \(\mu_t\), \(\bar{\mu}_t\), é dada pelo cálculo da odometria. 

A primeira linha do algoritmo pode ser entendida como o modelo de erro da odometria:
...
A matriz \(H_t\). No cenário proposto, os sensores são capazes de estimar a posição de \textit{landmarks} cuja posição real é conhecida anteriormente. A matriz \(Q\) representa a distribuição dos erros das observações. Portanto, para cada \textit{landmark} que encontra-se em \((L_x, L_y)\), as matrizes \(H\) e \(Q\) recebem duas linhas:
...
\subsection{Implementação}
A implementação do filtro extendido de Kalman encontra-se na classe \textit{KalmanFilterPoseUpdater}. É importante notar que este componente utiliza o cálculo da odometria e sempre realiza a atualização do último valor calculada pela mesma. 
...
A figura \ref{fig:efk1} mostra a comparação entre a posição estimada e a posição real do robô quando apenas uma base é utilizada. Pode-se observar a evidente melhoria no trajeto calculado em comparação com a figura \ref{fig:fig0}. Porém também fica evidente que em alguns pontos o erro na predição da rota é maior do que em outros. 
